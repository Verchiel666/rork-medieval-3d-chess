# 王翼弃兵 — 中世纪 3D 国际象棋（简体中文版）

> 本仓库是 [alexngdev99/rork-medieval-3d-chess](https://github.com/alexngdev99/rork-medieval-3d-chess) 的**简体中文汉化版**。所有面向用户的文本（菜单、按钮、对话框、提示、状态栏、帮助说明）与全部代码注释均已翻译为简体中文；代码逻辑、变量/函数/类名、文件结构与原项目完全一致。

![主菜单](acceptance-shots/01-main-menu.png)
![对局界面](acceptance-shots/04-game.png)

**汉化说明**：项目无 i18n 机制，采用硬编码替换；保留全部格式占位符与转义字符；已通过 `tsc` 0 错误、`vitest` 22/22 通过、`vite build` 成功，并经真实浏览器实测（无英文残留、无乱码、无文字截断、无布局错乱）。运行方式与原版相同：`cd web && npm install && npm run dev`。
